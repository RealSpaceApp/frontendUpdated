...
const ManageMembers = () => {
  ...
  const fetchCircleMembers = useCallback(async () => {
    try {
      ...
      if (response.status === 202) {
        ...
        console.warn('Failed to fetch circle members');
        setMembers([]);
      }
    } catch (error) {
      console.error('Error fetching circle members:', error);
      Alert.alert('Error', 'Failed to fetch circle members. Please try again later.');
    }
  }, [id]);
  ...
  useEffect(() => {
    fetchEventData();
    fetchCircleMembers();
  }, [fetchEventData, fetchCircleMembers]);
  ...
  const nonModerators = members.filter(member => member.moderator);
  const filteredNonModerators = nonModerators.filter(friend =>
    (friend.name?.toLowerCase() || '').includes(searchTerm.toLowerCase())
  );

  const filteredMembers = nonModerators.filter(member =>
  ...
  const removeMember = async (memberId) => {
    try {
      ...
      if (response.status === 202) {
        ...
        console.warn('Failed to remove member');
      }
    } catch (error) {
  ...
  const makeModerator = async (memberId) => {
    try {
      ...
      if (response.status === 202) {
        ...
        console.warn('Failed to make member a moderator');
      }
    } catch (error) {
      console.error('Error making member a moderator:', error);
      Alert.alert('Error', 'Failed to make member a moderator. Please try again later.');
    }
  };
      ...
      <Modal
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalText}>Are you sure you want to remove {Object.keys(selectedItems).length} members from Groove Gurus?</Text>
            <TouchableOpacity
              onPress={() => setModalVisibleRemoveFriend(false)}>
        ...
        <View style={styles.modalBackground2}>
          <View style={styles.modalContainer2}>
            <View style={styles.header}>
              ...
              <View style={styles.buttons}>
                <TouchableOpacity onPress={() => setModalVisible(false)} style={styles.selectButton2}>
